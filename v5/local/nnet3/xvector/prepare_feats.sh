...
if [[ $(hostname -f) == terra.hir.is ]] && [ ! -d $featdir/storage ]; then
  utils/create_split_dir.pl \
    /work/$USER/kaldi-data/egs/is_diarization/v5/xvector-$(date +'%m_%d_%H_%M')/xvector_cmvn_feats/storage $featdir/storage
fi
