...
# Apache 2.0.
#
# This script prepares the Ruv-di src. It will work as the train, dev, and
# eval sets of the ruvdi recipe. 
...
while read reco; do
  ...
  if [ -z "${path// }" ]; then
    ...
    echo "$reco sox -twav - -c1 -esigned -r8000 -G -twav - < $path |"
  fi
  count=$((count+1))
...
utils/copy_data_dir.sh $data_dir/ruvdi $data_dir/ruvdi3

utils/shuffle_list.pl $data_dir/ruvdi/wav.scp | head -n 17 \
...
utils/filter_scp.pl --exclude $data_dir/ruvdi1/wav.scp \
  $data_dir/ruvdi/wav.scp | head -n 3 > $data_dir/ruvdi2/wav.scp
utils/fix_data_dir.sh $data_dir/ruvdi2
utils/filter_scp.pl --exclude $data_dir/ruvdi1/wav.scp \
  $data_dir/ruvdi/wav.scp | tail -n 3 > $data_dir/ruvdi3/wav.scp
utils/fix_data_dir.sh $data_dir/ruvdi3

for dataset in ruvdi1 ruvdi2 ruvdi3; do
